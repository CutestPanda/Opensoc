...
void apb_uart_enable_itr(ApbUART* uart, uint8_t itr_mask, const ApbUartItrThConfig* config){
	...
	*LocalAddr = 0x00000001 | (itr_mask << 1);
	
	uart->itr_en = itr_mask;
	
	LocalAddr = &(uart->hardware->tx_itr_th);
	*LocalAddr = ((uint32_t)(config->tx_bytes_n_th - 1)) | (((uint32_t)(config->tx_idle_th - 2)) << 16);
	
	LocalAddr = &(uart->hardware->rx_itr_th);
	*LocalAddr = ((uint32_t)(config->rx_bytes_n_th - 1)) | (((uint32_t)(config->rx_idle_th - 2)) << 16);
}
