...
typedef struct{
	...
	uint32_t prescale; // 预分频系数 - 1(备份值)
	uint32_t auto_load; // 自动装载值 - 1(备份值)
	uint8_t cap_cmp_sel; // 捕获/比较选择(备份值)
	uint8_t chn_n; // 通道数
}ApbTimer;
...
void init_apb_timer(ApbTimer* timer, uint32_t base_addr, const ApbTimerConfig* config); // 初始化APB-TIMER
